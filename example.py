...
from Libs.mqtt_funcs import  *
from Libs.aruco_funcs import *
from Libs.visualization_tools import *
...
if __name__== "__main__":
    ...
    while RUN:
        ...
        if np.all(ids != None):
            ...
            for id in ids:
                direction_point, cntr = detect_marker_direction(corners[0])
                CHECKPOINT = is_it_checkpoint(cntr, dst_pt)
                direction_point = find_marker_direction_point(direction_point, cntr)
                abs_error_angle = find_abs_angle_error(direction_point, cntr, dst_pt)
                error_angle = find_angle_sign(direction_point, cntr, dst_pt, abs_error_angle)
                draw_direction(img, cntr, direction_point)
                draw_trajectory(img, cntr, dst_pt)
